...
func (m *DBModel) getGenresMovie(mv *Movie) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	genreQuery := `select 
			...
			where mg.movie_id = $1
			`
	genreRows, _ := m.DB.QueryContext(ctx, genreQuery, mv.ID)
...
func (m *DBModel) UpdatetMovie(movie Movie) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
